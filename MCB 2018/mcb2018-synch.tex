...
\]
We will assume where $F:\reals_+ \rightarrow \complex$ and $F(|z|) \rightarrow 0$ as $|z|\rightarrow \infty$
...
\begin{itemize}
	...
	\item $F$ is smooth and $F(|z|)\rightarrow 0$ as $|z|\rightarrow\infty$ ,
	\item The mean field $z$ is often a useful measure of coherence.
\end{itemize}
\vfill
\end{frame}
\section{Limit Cycle Dynamics}
...
\begin{tcolorbox}[notitle, boxrule=0pt, colback=lred]
\begin{theorem}
	Let $g$ be a even symmetric probability distribution, non-increasing on $[0,\infty)$, let $A = \{\lambda: \Re[\lambda] > 1\} \subset\complex$ and $E = 1/f(A)\subset\complex$. Then amplitude death is unstable with probability $\rightarrow 1$ as $n\rightarrow \infty$ if and only if $F() \in E$.
\end{theorem}
\end{tcolorbox}
}
\end{frame}
...
\begin{tcolorbox}[notitle, boxrule=0pt, colback=lred]
\begin{theorem}
	Let $g(\omega)$ be an even symmetric probability distribution, non-increasing on $[0,\infty)$. Then the dispersion function $f: \complex_+\rightarrow Y \subset\complex_+$ is a conformal map.
\end{theorem}
\end{tcolorbox}
}
...
\end{itemize}}
\begin{center}
	\includegraphics[scale=0.85]{scm2}
\end{center}	
\end{frame}
...
\begin{itemize}
	...
	\item coupled topology (via the nature of $F$)
\end{itemize}
\end{frame}
\begin{frame}{Demonstration of Node dynamics.}
\centering
\vfill
